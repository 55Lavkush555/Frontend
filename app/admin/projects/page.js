...
export default function AdminProjects() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(
        isEditing ? `/api/projects/${editingSlug}` : '/api/projects',
        {
  ...
  const toggleFeatured = async (project) => {
    setMessage('');
    setError('');
    const res = await fetch(`/api/projects/${project.slug}`, { method: 'PATCH' });
    const data = await res.json();
    if (!data.success) { setError(data.error || 'Failed to update'); return; }
    setMessage(data.data.project.featured ? 'Marked as featured.' : 'Removed from featured.');
    fetchProjects();
  };
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-6 mb-8 shadow-sm">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1.5 uppercase tracking-wide">Project Name *</label>
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1.5 uppercase tracking-wide">GitHub URL</label>
                  <input type="url" name="githubUrl" value={form.githubUrl} onChange={handleChange}
                    placeholder="https://github.com/..."
                    className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-gray-400 transition-colors" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1.5 uppercase tracking-wide">Live URL</label>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1.5 uppercase tracking-wide">Description (Markdown) *</label>
                <textarea name="description" required rows={8} value={form.description} onChange={handleChange}
              ...
              <button type="submit" disabled={submitting}
                ...
                {submitting ? 'Saving...' : editingSlug ? 'Update Project' : 'Add Project'}
              </button>
            </form>
          </div>
        )}
